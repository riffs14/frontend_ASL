...
const Home = () => {
  ...
  const [monthlyTotalAmountData, setMonthlyTotalAmountData] = useState([]); // New state for total amount data
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      bookingSnapshot.docs.forEach(docSnap => {
        ...
        // ----- monthly total amount aggregation -----
        if (bdForThisMonth) {
          ...
          if (monthlyTotalMap.has(key)) {
            ...
            if (Number(data.booking_type) === 0) { // New Admission
              row.newAdmissions += amount;
            } else if (Number(data.booking_type) === 1) { // Repayment
              row.repayments += amount;
            }
      ...
      // Build chart data array for total amount collected (last 12 months)
    ...
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', gap: '20px' }}>
      <div style={{ flex: 1, maxWidth: '600px' }}>
        ...
        <div style={{ display: 'flex', gap: '20px' }}>
          ...
          {/* Total Amount Collected by Month Chart */}
          <div style={{ width: '48%' }}>
            <h2>Total Amount by Month (last 12 months)</h2>
            <div style={{ width: '100%', height: 300 }}>
              <ResponsiveContainer>
                <RBarChart data={monthlyTotalAmountData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="month" />
                  <YAxis allowDecimals={false} />
                  <Tooltip />
